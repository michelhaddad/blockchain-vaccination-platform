...
3. [Start / stop a Hyperledger Fabric network using `basic` configuration](#start--stop-the-basic-network).
...
We will use the `basic` Hyperpedger Fabric network. This is the network that contains all the organizations
in our Blockchain Vaccination Platform, namely Manufacturer, Border Control, Central Inspection (aka Impact), Hopspital, Storage Facility, Donor, and the Ministry Of Public Health. Each organization is represented by one unique peer. The network contains a solo orderer 
...
If you make any changes to chaincode, you will need to remove the old images:

```
make rmchaincode
```

## Create users and start the Express REST server

The vaccination-web-api application is used to create users, but more importantly it is used to communicate with the Blockchain through simple HTTP requests. The transactions generated by these requests can also be analyzed with the Elastic stack.

The commands in this section should be issued from the `blockchain-analyzer/apps/vaccination-web-api` directory.
...
To start the Express server and start sending requests, run the following command:
```
npm start
```
...
The commands in this section should be issued from the `blockchain-analyzer/apps/vaccination-web` directory.
